...
public class EnrichedViewModel extends ViewModel {	
	...
	protected Collection<Probe> extendProbeSelection(Probe pb) {		
		String name = (pb instanceof DerivedProbe)?((DerivedProbe)pb).getSourceName():pb.getName();
		MasterTable mt = getDataSet().getMasterTable();
		if (mt instanceof DerivedMasterTable)
			return ((DerivedMasterTable)getDataSet().getMasterTable()).getProbes(name);
		else 
			return Arrays.asList(new Probe[]{mt.getProbe(name)});
	}
