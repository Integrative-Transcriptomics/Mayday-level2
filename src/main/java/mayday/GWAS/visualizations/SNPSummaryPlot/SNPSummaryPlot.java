...
import mayday.GWAS.utilities.ATCGColors;
...
public class SNPSummaryPlot extends RevealVisualization {
	...
	protected void calculateViewElements() {
		...
		persBars = new IndividualPersonBar[numSNPs];
		
		int pIndex = setting.getPerson().getIndex();
		Haplotypes h = haplotypesList.get(pIndex);
		
		for(int i = 0; i < snpList.size(); i++) {
			SNP s = snpList.get(i);
			int snpIndex = s.getIndex();
			...
			char allelA = h.getSNPA(snpIndex);
			char allelB = h.getSNPB(snpIndex);
			persBars[i] = new IndividualPersonBar(allelA, allelB);
		}
	}
	
	protected void updatePersonBars() {
		int pIndex = setting.getPerson().getIndex();
		Haplotypes h = haplotypesList.get(pIndex);
		for(int i = 0; i < snpList.size(); i++) {
			int si = snpList.get(i).getIndex();
			char allelA = h.getSNPA(si);
			char allelB = h.getSNPB(si);
			persBars[i] = new IndividualPersonBar(allelA, allelB);
		}
	}
