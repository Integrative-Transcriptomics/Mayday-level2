...
public class DCPMSummary extends AbstractTransformationPlugin {
	...
	public void compute() {		
		for (Experiment e : transMatrix.getExperiments(this)) {
			...
			transMatrix.setIntermediateData(e, 
					new DCPMSummaryExperimentData(e, (LocusReadCountData)ex, transMatrix.getInputState(this, e).getNumberOfFeatures())
			);
		}	
	}
	...
	public PluginInfo register() throws PluginManagerException {
		return new PluginInfo(
				this.getClass(), 
				MC+".CombineReads.DCPM", 
				new String[0], 
				MC, null, 
				"Florian Battke", 
				"battke@informatik.uni-tuebingen.de", 
				"Combine read counts into an expression level for a given locus using the \"depth of coverage per base per million reads\" measure.", 
		"Combine Read Counts (DCPM) -- depth of coverage per base per million reads");
	}
