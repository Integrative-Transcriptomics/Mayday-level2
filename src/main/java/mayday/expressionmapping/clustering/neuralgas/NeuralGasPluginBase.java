 ...
 *
 */
public abstract class NeuralGasPluginBase extends ClusterPlugin {
	 ...
	 */
	public List<ProbeList> runWithSettings(List<ProbeList> probeLists, MasterTable masterTable, NeuralGasSetting settings) {
	...
	public List<ProbeList> cluster(List<ProbeList> probeLists, MasterTable masterTable,
		...
		NeuralGasClustering NGClustering = new NeuralGasClustering(matrix, settings);
		
		ClusterTask cTask = new ClusterTask("Neural Gas Clustering");
		cTask.setClAlg(NGClustering);
		NGClustering.setClusterTask(cTask);
		
		cTask.start();
		cTask.waitFor();
		
		int [] ClusterIndices = cTask.getClResult();
		NGClustering.setClusterTask(null);
	...
	private List<ProbeList> createResultProbeList(MasterTable masterTable,
		...
		for(int i = 0; i < settings.getNumOfClusters(); i++) {
			...
			tempList.setName("NeuralGas" + " " + (settings.getNumOfClusters()-i));
			tempList.setAnnotation(new AnnotationMIO(
					"Neural Gas Clustering, NG neighbor parameter = "
					+ ((NeuralGasSetting)settings).getNeighborParameter() + ", maximal number of rounds = "
					+ settings.getMaxNumOfRounds() + ", Dimension = " + settings.getDimension(), ""));
